...
def main():
    ...
    logging_fmt = "%(asctime)s %(process)d %(processName)s  - %(message)s"
    logging.basicConfig(filename="disruptcnn.log", level=logging.DEBUG, format=logging_fmt)
    logging.debug(f" rank {args.rank}: Starting main")
...
def main_worker(gpu,ngpus_per_node,args):
    ...
    dataset = EceiDataset(data_root,clear_file,disrupt_file,
                          ...
                          nsub=args.nsub,nrecept=args.nrecept, rank=args.rank)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx, it in enumerate(dali_iter):
            ...
            nvtx.range_pop()
            logging.debug(f"rank {args.rank:02d} - Batch {batch_idx:03d} end")
        logging.debug(f"rank {args.rank:02d} - Epoch {epoch:02d} end")
        nvtx.range_pop()
